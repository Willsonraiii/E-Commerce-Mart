  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-[1.8rem] text-paper">Offers</h1>
          <p className="mt-1 text-[.88rem] text-mint/50">The three deal cards on the homepage.</p>
        </div>
      ...
      {loading ? (
        <div className="grid gap-3.5 lg:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-48 animate-pulse rounded-[20px] bg-white/6" />)}
        </div>
      ) : (
        <div className="grid gap-3.5 lg:grid-cols-3">
          {items.map((o, i) => {
            ...
            return (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-[20px] border border-white/8 p-5"
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="mb-2 flex items-start justify-between gap-2">
                    <span className="text-[.72rem] font-semibold uppercase tracking-wider text-mint/50">{o.kicker}</span>
                    <span className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <button onClick={() => { setForm({ ...o, to: o.to || o.to_path || '/shop' }); setOpen(true) }} aria-label="Edit"
                        className="grid h-7 w-7 place-items-center rounded-lg border border-white/15 bg-black/25 text-mint hover:bg-black/45">
                        <EditIcon size={13} />
                      </button>
                      <button onClick={() => remove(o)} aria-label="Delete"
                        className="grid h-7 w-7 place-items-center rounded-lg border border-white/15 bg-black/25 text-[#f0a58a] hover:bg-terracotta/40">
                        <TrashIcon size={13} />
                      </button>
                    </span>
                  </div>
                  <p className="font-display text-[1.35rem] text-paper">{o.title}</p>
                  <Badge color={t.from} className="mt-1.5">{o.tag}</Badge>
                  <p className="mt-2.5 text-[.84rem] leading-relaxed text-mint/65">{o.detail}</p>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Offer card" wide>
        <form onSubmit={save} className="grid gap-3.5 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5 sm:col-span-2">
            <span className="text-[.78rem] text-mint/60">Theme</span>
